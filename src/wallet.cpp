...
// Paris: optional setting to unlock wallet for block minting only;
...
// Paris: total coins staked (non-spendable until maturity)
       ...
       BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
       {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                if (pcoin->nTime > nSpendTime)
                    continue;  // Paris: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            loop
            {
                ...
                // Paris: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    else
                    {
                        ...
                        if (!GetBoolArg("-avatar")) // Paris: not avatar mode
                        {
...
// Paris: create coin stake transaction
...
// Paris: check 'spent' consistency between wallet and txindex
// Paris: fix wallet spent state according to txindex
...
// Paris: disable transaction (only for coinstake)
