...
static const int STAKE_MAX_AGE = 60 * 60 * 24 * 365 * 1; // stake age of full weight 1 years
...
public:
    ...
    {
        // Paris: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    bool GetCoinAge(CTxDB& txdb, uint64& nCoinAge) const;  // Paris: get transaction coin age
...
public:
    ...
    // Paris: block signature - signed by coin base txout[0]'s owner
    ...
    // Paris: two types of block: proof-of-work or proof-of-stake
    ...
    // Paris: get max transaction timestamp
    ...
    bool AcceptBlock();
    bool GetCoinAge(uint64& nCoinAge) const; // Paris: calculate total coin age spent in block
    bool SignBlock(const CKeyStore& keystore);
    bool CheckBlockSignature() const;
    unsigned int GetStakeEntropyBit() const; // Paris: entropy bit for stake modifier if chosen by modifier
...
public:
    ...
    unsigned int nFile;
    unsigned int nBlockPos;
    CBigNum bnChainTrust; // Paris: trust score of block chain
    int nHeight;
    int64 nMint;
    int64 nMoneySupply;

    unsigned int nFlags;  // Paris: block index flags
    enum  
    {
