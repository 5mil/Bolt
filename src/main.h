...
static const int64 MAX_MINT_PROOF_OF_WORK = 20000 * COIN;
...
static const int STAKE_MIN_AGE = 60 * 60 * 6 * 1; // minimum age for coin age 2 days
static const int STAKE_MAX_AGE = 60 * 60 * 24 * 365 * 6; // stake age of full weight 6 years
...
public:
    ...
    {
        // Bolt: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    bool GetCoinAge(CTxDB& txdb, uint64& nCoinAge) const;  // Bolt: get transaction coin age
...
public:
    ...
    // Bolt: block signature - signed by coin base txout[0]'s owner
    ...
    // Bolt: two types of block: proof-of-work or proof-of-stake
    ...
    // Bolt: get max transaction timestamp
    ...
    bool AcceptBlock();
    bool GetCoinAge(uint64& nCoinAge) const; // Bolt: calculate total coin age spent in block
    bool SignBlock(const CKeyStore& keystore);
    bool CheckBlockSignature() const;
    unsigned int GetStakeEntropyBit() const; // Bolt: entropy bit for stake modifier if chosen by modifier
...
public:
    ...
    unsigned int nFile;
    unsigned int nBlockPos;
    CBigNum bnChainTrust; // Bolt: trust score of block chain
    int nHeight;
    int64 nMint;
    int64 nMoneySupply;

    unsigned int nFlags;  // Bolt: block index flags
    enum  
    {
